...
def sign_name(video_capture, frame, sign_path, name):
	...
	reply = g.buttonbox('打卡确认', image=record_image, choices=choices)
...
def face_detection_sign(video_capture, sign_path):
	...
	while True:
		...
		if len(face_names) == 0:
			...
			frame, sign_state = display_result_sign(video_capture, frame, face_locations, face_names, sign_path)
			# cv2.imshow('Video', frame)
			if sign_state:
				break
